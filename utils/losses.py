...
class IntraSCL(nn.Module):
    ...
    def forward(self, features, targets):
        ...
        cls_num_list = self.cls_num_list.to(device=device)
        cls_num_list = cls_num_list / torch.sum(cls_num_list)
        ...
        temp = cls_num_list.gather(0, targets.view(-1)).view(-1, 1)
        temp = temp.mm(temp.T)
...
class InterSCL(nn.Module):
    ...
    def forward(self, centers1, features, targets):
        ...
        # keep on same device as targets to avoid CPU/GPU indexing mismatch
        batch_cls_count = torch.eye(len(self.cls_num_list), device=device)[
